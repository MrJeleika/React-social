...
export const APIgetMyProfile = () => {
  return instance.get(`auth/me`).then(response =>{
   return instance.get(`profile/${response.data.data.id}`).catch(err => console.log(err)).then(response => {
      return response.data
    })
  })
}
...
export const APIgetProfileStatus = () => {
  return instance.get(`auth/me`).then(response =>{
    return instance.get(`profile/status/${response.data.data.id}`).then(response => {
       return response.data
     })
   })
}
